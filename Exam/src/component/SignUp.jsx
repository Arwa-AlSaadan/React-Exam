import React from 'react'
import { useNavigate } from 'react-router-dom';

export default function SignUp() {

    const navigate = useNavigate();
    const [name,setName] = React.useState("");
    const [email,setEmail] = React.useState("");
    const [Password,setPassword] = React.useState("");
    const [reEnterPass,setReEnterPass] = React.useState("");


    const Register=()=>{
     
        const emailCheck = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;

        if(name.length<4){

        }else if(!emailCheck.test(email) || email.length<10 ){

        }else if(Password.length<=5 || Password !== reEnterPass){

        }else{
            fetch('https://fakestoreapi.com/users',{
            method:"POST",
            body:JSON.stringify(
                {
                    email: email,
                    username: name,
                    password:Password,
                    name:{
                        firstname:"",
                        lastname:""
                    },
                    address:{
                        city: "",
                        street: "",
                        number:"",
                        zipcode:"",
                        geolocation:{
                            lat:"",
                            long:""
                        }
                    },
                    phone:""
                }
            )
        })
            .then(res=>res.json())
            .then(json=>console.log(json))

              navigate("/Login")

        }


    }

  return (
    <>
    <div className='flex flex-col items-center h-screen'>
     <img className='h-[6vh] mt-6' src='data:image/png;base64,'></img>

    <div className='flex flex-col  h-fit w-[30vw] mt-4 border rounded-md'>
    <div className='flex flex-col gap-2 p-4'>
    <p className='font-bold text-xl'>Create account</p>
    <label>Your name</label>
    <input value={name} onChange={(e)=>setName(e.target.value)} className="border py-2 rounded-md pl-2 pr-2" placeholder='First and last name'></input>
    <label>Mobile number or email</label>
    <input value={email} onChange={(e)=>setEmail(e.target.value)} className="border py-2 rounded-md pl-2 pr-2" placeholder=''></input>
    <label>Password</label>
    <input type="password" value={Password} onChange={(e)=> setPassword(e.target.value)} className="border py-2 rounded-md pl-2 pr-2" placeholder='At least 6 characters'></input>
    <label>Re-enter Password</label>
    <input type="password"  value={reEnterPass} onChange={(e)=> setReEnterPass(e.target.value)} className="border py-2 rounded-md pl-2 pr-2"></input>
    <button onClick={Register} className='bg-[rgb(255,216,20)] className="border py-2 rounded-md pl-2 pr-2"'>Countinue</button>
    </div>

    </div>

    </div>
    
    </>
  )
}
