...
export default function Login() {
    ...
    const Login=()=>{
        ...
        .then(res=> {
            const foundeUser= res.data.find((item)=> item.email === email && item.password === password)

            if (foundeUser){
                localStorage.setItem("userName",foundeUser.name.firstname)
                localStorage.setItem("userId",foundeUser.id)
  ...
  return (
    ...
    <input type="password" value={password} onChange={(e)=> setPassword(e.target.value)} className="border py-2 rounded-md pl-2 pr-2"></input>
    <button onClick={Login} className='bg-[rgb(255,216,20)] className="border py-2 rounded-md pl-2 pr-2"'>Login</button>
    </div>
